...
As it can be seen, the out of sample error rate is NUMBER.

We can also use the original test data to generate our 20 predictions.

```{r finalPredictions, cache=TRUE, warning=TRUE, message=TRUE}
final_predictions <- predict(modFit, testing_clean)
final_predictions_confusion_matrix <- confusionMatrix(final_predictions, testing_clean$classe)
print(final_predictions_confusion_matrix)
```
